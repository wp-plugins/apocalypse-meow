...
if($meow_fail_reset_on_success && count($meow_banned))
{
	...
	if($wpdb->num_rows)
	{
		...
		foreach($dbResult AS $Row)
		{
			$meow_last_successful = (int) $wpdb->get_var("SELECT MAX(`date`) FROM `{$wpdb->prefix}meow_log` WHERE `ip`='" . mysql_real_escape_string($Row['ip']) . "' AND `success`=1");
			//this person isn't banned after all...
			if($meow_fail_limit > (int) $wpdb->get_var("SELECT COUNT(*) AS `count` FROM `{$wpdb->prefix}meow_log` WHERE `ip`='" . mysql_real_escape_string($Row['ip']) . "' AND `success`=0 AND UNIX_TIMESTAMP()-`date` <= $meow_fail_window AND `date` > $meow_last_successful"))
